...
### Sintaxis
```
R#show ip interface [nombre interfaz][numero]
```
Abreviatura: `sh ip int `
### Descripción de propósito/uso

Muestra la configuración técnica absoluta y detallada de una **interfaz en especifico** las interfaces del equipo. Te mostrará por cada puerto: la IP, la máscara, si tiene listas de acceso (ACL) aplicadas, el estado del proxy ARP, temporizadores, configuraciones de seguridad y características de compresión.
### Ejemplo
```
R#show ip interface GigabitEthernet 0/1
```
O abreviado:
```
R#sh ip int g0/1
